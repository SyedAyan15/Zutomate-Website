...
- Never write Tailwind utility classes — Tailwind is an unused dependency here and is not wired into PostCSS, so they silently do nothing. Style in `app/globals.css`
...
- Always reference logos and photos by `/assets/...` path from `public/` (the base64-embedding rule was a static-site preview workaround and does not apply here)
...
- Browser-only work belongs in a `'use client'` component inside `useEffect`, with cleanup (the IIFE rule was for the old static `script.js`)
